...
Found 8 regressions caused by the non-USE_TYPO OS/2 path dropping hhea `lineGap` for TNR/Arial. Fixed by adding `face.line_gap()` to the Windows metrics path. The `0.75`/`1.2`/`+0.5` fallback constants were not involved. Zero regressions after fix. See `plan_archery_progress.md` for full analysis.

### Step 3: Update handcrafted fixture expectations ✅ COMPLETED

Some handcrafted cases (case1-case37) may have been designed with the wrong line height. After fixing regressions, update any stored score baselines, but only change case fixtures if they genuinely are wrong, not because we want better scores!

### Step 4: Un-skip the polish_archery fixture ✅ COMPLETED

Score after OS/2 fix: **15.0% Jaccard / 48.4% SSIM** — unchanged from before (the OS/2 fix was net-neutral for this fixture since TNR's win metrics + hhea lineGap = original hhea metrics). Since score remains below 20% Jaccard, the fixture stays on the SKIPLIST. The vertical drift issue for this fixture is NOT caused by font metrics — further investigation needed (likely paragraph spacing, table layout, or other layout differences).

Features used by this fixture (all implemented):
- Exact line spacing ✅
