...
1. Select from the top the first unsolved problem from tests/output/annotations.json. You are to work on ONE and ONLY ONE annoation. Update progress file with the selected annotation when selected. \
...
4. If you believe the problem is fixed, set the fixed boolean in annotations.json for that case to true. DO NOT forget to set it to fixed. If a problem is unfixable, make a note of what you have learned in the progress-file.
